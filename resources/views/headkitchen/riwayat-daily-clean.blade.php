            ...
            <div class="card border-0 shadow-sm">
                <div class="card-body">
                    <div class="table-responsive">
                        <form id="bulkDeleteForm" method="POST" action="{{ route('headkitchen.riwayat.daily-clean.bulk-delete') }}">
                            @csrf
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <div class="d-flex gap-1">
                                                    <a href="{{ route('headkitchen.riwayat.daily-clean.detail', $rec['id']) }}" class="btn btn-sm btn-outline-info">
...
function openPhotos(id) {
    ...
    fetch("{{ route('headkitchen.riwayat.daily-clean.detail-json', 0) }}".replace('/0', '/' + id))
...
function openSingleDeleteModal(id, tanggal, shift, barista, jumlahFoto) {
    ...
    swalConfirm({
        ...
        showLoader: true,
        preConfirm: function () {
            return fetch("{{ route('headkitchen.riwayat.daily-clean.delete', 0) }}".replace('/0', '/' + id), {
                method: 'POST',
                headers: {
                ...
                form.method = 'POST';
                form.action = '{{ route("headkitchen.riwayat.daily-clean.delete", 0) }}'.replace('/0', '/' + id);
...
function openBulkDeleteModal() {
    ...
    swalConfirm({
        ...
        showLoader: true,
        preConfirm: function () {
            return fetch("{{ route('headkitchen.riwayat.daily-clean.bulk-delete') }}", {
                method: 'POST',
                headers: {
                    'X-Requested-With': 'XMLHttpRequest',
                    'Accept': 'application/json'
