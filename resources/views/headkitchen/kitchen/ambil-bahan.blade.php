...
@section('title', 'Ambil Bahan Gudang - ' . config('branding.app_name'))
    ...
    <div class="row mb-4">
        <div class="col-12">
            <div class="card border-0 shadow-sm">
                <div class="card-body">
                    <h2 class="mb-0"><i class="bi bi-box-arrow-right me-2"></i>Ambil Bahan Gudang</h2>
                    <p class="text-muted mb-0">Catat pengambilan stok dari Gudang ke Kitchen</p>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="col-12">
            <div class="card border-0 shadow-sm">
                <div class="card-header">
                    <h5 class="mb-0"><i class="bi bi-pencil-square me-2"></i>Form Ambil Bahan Gudang</h5>
                </div>
                    ...
                    <div class="card border-0 shadow-sm mb-4">
                        <div class="card-body">
                            ...
                            <p class="card-text text-muted mb-0">
                                Input jumlah bahan yang diambil dari Gudang. Kosongkan input atau isi 0 jika tidak ada pengambilan untuk bahan tersebut. Jumlah pengambilan tidak boleh melebihi Stok Gudang.
                            </p>
                        </div>
                    </div>

                    <form method="POST" action="{{ route('headkitchen.kitchen.ambil-bahan.store') }}" id="updateStokForm">
                        @csrf
                        <div class="row mb-4">
                            ...
                            @endif
                            <div class="col-md-{{ session('role') === 'manajemen' ? '6' : '4' }} mb-3">
                                <label for="Kitchen" class="form-label"><i class="bi bi-person me-1"></i>Nama <span class="text-danger">*</span></label>
                                                                ...
                                                                <div class="barang-grid">
                                                                    @foreach ($grp['items'] as $item)
                                                                        <div class="barang-card input-with-unit">
                                                                            ...
                                                                            <input type="text" class="form-control form-control-sm item-input" id="{{ $item['kode'] }}" name="{{ $item['kode'] }}" value="{{ old($item['kode'], '') }}" inputmode="decimal" placeholder="0" data-max="{{ $gudang_stocks[$item['kode']] ?? 0 }}">
...
document.getElementById('updateStokForm').addEventListener('submit', function(e) {
    ...
    if (!hasValue) {
        e.preventDefault();
        showStokError('Minimal satu bahan harus diambil (jumlah > 0).');
        return false;
    }
    ...
    if (invalidNum) {
        e.preventDefault();
        hideStokError();
        Swal.fire({ icon: 'warning', title: 'Periksa Kembali', text: invalidNum.id.replace(/_/g, ' ').toUpperCase() + ' harus berupa angka positif.', confirmButtonColor: '#DC3545', background: '#1F2026', color: '#e4e4e7', borderRadius: '16px', customClass: { popup: 'swal-dark-popup' } });
        invalidNum.focus();
        return false;
    }
    ...
    if (exceedsMax) {
        e.preventDefault();
        hideStokError();
        const max = exceedsMax.getAttribute('data-max');
        Swal.fire({ icon: 'error', title: 'Stok Tidak Cukup', text: 'Pengambilan ' + exceedsMax.id.replace(/_/g, ' ').toUpperCase() + ' melebihi stok gudang yang tersedia.', confirmButtonColor: '#DC3545', background: '#1F2026', color: '#e4e4e7', borderRadius: '16px', customClass: { popup: 'swal-dark-popup' } });
        exceedsMax.focus();
        return false;
    }
});
...
document.querySelectorAll('.item-input').forEach(function(input) {
    input.addEventListener('input', function(e) {
        ...
        if (document.querySelectorAll('.item-input.is-invalid').length === 0) hideStokError();
    });
});
</script>
@endpush
