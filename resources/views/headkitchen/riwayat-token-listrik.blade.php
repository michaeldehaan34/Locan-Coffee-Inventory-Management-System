    ...
    <div class="row mb-3">
        <form method="GET" class="row g-2">
            ...
            <div class="col-md-2">
                ...
                <input type="text" class="form-control" name="barista" placeholder="Nama Kitchen..." value="{{ $filter_barista }}">
            </div>
            ...
            <div class="card border-0 shadow-sm">
                <div class="card-body">
                    <div class="table-responsive">
                        <form id="bulkDeleteForm" method="POST" action="{{ route('headkitchen.riwayat.token-listrik.bulk-delete') }}">
                            @csrf
...
function openSingleDeleteModal(id, tanggal, shift, barista) {
    ...
    swalConfirm({
        ...
        showLoader: true,
        preConfirm: function () {
            return fetch("{{ route('headkitchen.riwayat.token-listrik.delete', 0) }}".replace('/0', '/' + id), {
                method: 'POST',
                headers: {
                ...
                form.method = 'POST';
                form.action = '{{ route("headkitchen.riwayat.token-listrik.delete", 0) }}'.replace('/0', '/' + id);
...
function openBulkDeleteModal() {
    ...
    swalConfirm({
        ...
        showLoader: true,
        preConfirm: function () {
            return fetch("{{ route('headkitchen.riwayat.token-listrik.bulk-delete') }}", {
                method: 'POST',
                headers: {
                    'X-Requested-With': 'XMLHttpRequest',
                    'Accept': 'application/json'
